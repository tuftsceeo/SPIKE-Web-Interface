...
function Service_SPIKE() {
    ...
    /*
        ...
        gyro[0] - up/down detector ( top: 1000, bottom: -1000, neutral: 0)
    ...
    */
     ...
     * 
     * @param {string} command -  a string to send (or sequence of commands, separated by new lines)
     */
    ...
    * 
    *  Returns:
    * {lastUJSONRPC} (string) - a string represent a JSON object from UJSON RPC
    * 
    */
    ...
    */
    PrimeHub = function() {
        ...
        */
        ...
        motion_sensor = function () {
             ...
             */
            function wait_for_new_gesture(callback) {
             ...
             */
            function wait_for_new_orientation(callback) {
            ...
            return {
                was_gesture: was_gesture,
                wait_for_new_gesture: wait_for_new_gesture,
                wait_for_new_orientation: wait_for_new_orientation,
                get_yaw_angle: get_yaw_angle,
    ...
    Motor = function (port) {
        ...
        function run_to_position (degrees, speed) {
            if ( speed !== undefined && typeof speed == "number" ) {
                motorGoRelPos (port, degrees, speed, stallSetting, stopMethod);
            }
            else {
                motorGoRelPos(port, degrees, defaultSpeed, stallSetting, stopMethod);
            }
        }
        ...
        function start_at_power (power) {
            motorPwm (port, power, stallSetting);
        }
        ...
        function start (speed) {
            if (speed !== undefined && typeof speed == "number") {
                motorStart (port, speed, stallSetting);
            }
            else {
                motorStart(port, defaultSpeed, stallSetting);
            }
        }
        ...
        function run_for_seconds (seconds, speed) {
            if (speed !== undefined && typeof speed == "number") {
                motorRunTimed (port, seconds, speed, stallSetting)
            }
            else {
                motorRunTimed(port, seconds, defaultSpeed, stallSetting)
            }
        }
        ...
        function run_for_degrees (degrees, speed) {
            if (speed !== undefined && typeof speed == "number") {
                motorRunDegrees (port, degrees, speed, stallSetting);
            }
            else {
                motorRunDegrees(port, degrees, defaultSpeed, stallSetting);
            }
        }
    ...
    MotorPair = function(leftPort, rightPort) {
        ...
        function move (amount, unit, steering, speed) {
            ...
            if ( unit == "cm" || unit !== undefined ) {
                ...
                moveTankDegrees(degrees, leftSpeed, rightSpeed, leftPort, rightPort, 0);
            } 
            else if ( unit == "in" ) {
                ...
                moveTankTime(seconds, leftSpeed, rightSpeed, leftPort, rightPort, 0);
            }
        }
        ...
        function start (steering, speed) {
            ...
            moveTankSpeeds(leftSpeed, rightSpeed, leftPort, rightPort);
        }

        function stop() {
            moveTankPowers(0, 0, leftPort, rightPort);
        }
    ...
    async function motorGoRelPos(port, position, speed, stall, stop) {
        var randomId = Math.floor((Math.random() * 10000));
        var command = '{"i":' + randomId +
            ...
            ', "speed":' + speed +
            ', "stall":' + stall + // boolean
            ', "stop":' + 0 + // 'coast','brake','hold'
            '} }';
        sendDATA(command);
    }

    async function motorRunTimed(port, time, speed, stall) {
        var randomId = Math.floor((Math.random() * 10000));
        var command = '{"i":' + randomId +
    ...
    async function motorRunDegrees(port, degrees, speed, stall) {
        var randomId = Math.floor((Math.random() * 10000));
        var command = '{"i":' + randomId +
    ...
    async function PrimeHubEventHandler() {
        ...
        // gives orientation of the hub (leftside, up,..), tapping of hub, 
            ...
            if (isOrientationData) {
                ...
                funcAfterNewOrientation(newOrientation);
            }
