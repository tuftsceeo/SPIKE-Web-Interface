...
let initWindowHTML = `<p class="text-center text-lg" id="window-title">Hello!</p>
...
let displayTests = (title, testNames, devices) => {
...
let testMotorsWindow = () => {
    let testNames = ["All Motor & Motor Pair Tests", "All Motor Tests", "All Motor Pair Tests"];
    let devices = [["Motors", "MotorPair"], ["Motors"], ["MotorPair"]];
    displayTests("Motor + MotorPair Tests", testNames, devices);
};
let testHubWindow = () => {
    let testNames = ["All Hub Tests"];
    displayTests("Prime Hub Tests", testNames, [[]]);
};
let testMiscWindow = () => {
    let testNames = ["All Misc. Tests"];
    displayTests("Other Tests", testNames, [[]]);
};
let testSensorsWindow = () => {
    let testNames = ["All Sensor Tests", "Distance Sensor Tests", "Force Sensor Tests", "Color Sensor Tests"];
    let devices = [["DistanceSensor", "ForceSensor", "ColorSensor"], ["DistanceSensor"], ["ForceSensor"], ["ColorSensor"]];
    displayTests("Sensor Tests", testNames, devices);
};
// Connect to Service Dock and check for correct ports when test is selected
let setupWindow = (testName, devices) => {
    // UI Changes
...
let activateDevices = (spikeTests, devices) => {
};
